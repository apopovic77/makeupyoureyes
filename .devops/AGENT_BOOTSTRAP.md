...
1. **Repository context**
   - Project lives at `/var/code/makeupyoureyes`.
...
4. **Deployment target**
   - Built files go to `/var/www/makeupyoureyes.arkserver.arkturian.com`.
   - Backups are stored with prefix `makeupyoureyes` under `/var/backups/` (configurable via templates).

5. **Startup checklist for new session**
   - `cd /var/code/makeupyoureyes`.
