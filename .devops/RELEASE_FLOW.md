...
6. Validate the site at `https://makeupyoureyes.arkserver.arkturian.com` (if applicable) and confirm fresh assets reached `/var/www/makeupyoureyes.arkserver.arkturian.com`.

## Rollback Strategy
- Use `.devops/rollback.sh` on the server to restore from `/var/backups/makeupyoureyes-*`.
